use crate::db_diesel::{CompositeFilter, CompoundNameMixin, EphemeralOverlay, ScopeContext};
...
async fn test_find_symbol_by_name() -> anyhow::Result<()> {
    ...
    let empty_selection = index.find_symbol(&empty_filter, ScopeContext::Skip, ScopeContext::Skip, &EphemeralOverlay::empty()).await?;
    ...
    let selection = index.find_symbol(&a_filter, ScopeContext::Skip, ScopeContext::Skip, &EphemeralOverlay::empty()).await?;
    assert!(
    ...
    let selection = index.find_symbol(&main_filter, ScopeContext::Skip, ScopeContext::Skip, &EphemeralOverlay::empty()).await?;
    assert!(
        !selection.nodes.is_empty(),
    ...
    let compound_selection = index.find_symbol(&compound_filter, ScopeContext::Skip, ScopeContext::Skip, &EphemeralOverlay::empty()).await?;
    assert!(
...
async fn test_find_symbol_by_name_token_ordering() -> anyhow::Result<()> {
    ...
    let selection = index.find_symbol(&filter1, ScopeContext::Skip, ScopeContext::Skip, &EphemeralOverlay::empty()).await?;
    ...
    let selection = index.find_symbol(&filter2, ScopeContext::Skip, ScopeContext::Skip, &EphemeralOverlay::empty()).await?;
    let mut found_names: Vec<String> = selection
        .nodes
        .iter()
        .map(|node| node.symbol.name.clone())
    ...
    let reverse_selection = index.find_symbol(&filter3, ScopeContext::Skip, ScopeContext::Skip, &EphemeralOverlay::empty()).await?;
    assert!(
