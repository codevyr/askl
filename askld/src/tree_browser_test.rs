...
async fn tree_browser_root_returns_direct_children() {
    ...
    let result = store
        .list_project_tree_multi(1, &["/".to_string()], false)
        .await
        .unwrap();
...
async fn tree_browser_leaf_directory_returns_only_files() {
    let store = shared_test_store().await;
    let result = store
        .list_project_tree_multi(1, &["/src/util".to_string()], false)
        .await
        .unwrap();
    ...
    match result {
        MultiTreeResult::Nodes(mut map) => {
            ...
            assert_eq!(
                dir_nodes.len(),
                0,
                "/src/util should have no child directories"
            );
            ...
            assert!(
                file_paths.contains(&"/src/util/util.go"),
                "Should contain util.go"
            );
            assert!(
                file_paths.contains(&"/src/util/helper.go"),
...
async fn tree_browser_nonexistent_path_returns_not_directory() {
    let store = shared_test_store().await;
    let result = store
        .list_project_tree_multi(1, &["/nonexistent".to_string()], false)
        .await
        .unwrap();
...
async fn tree_browser_multi_path_returns_all_paths() {
    ...
    match result {
        MultiTreeResult::Nodes(mut map) => {
            ...
            let root_dirs: Vec<_> = root_nodes
                .iter()
                .filter(|n| n.node_type == NodeType::Dir)
                .collect();
            ...
            let src_dirs: Vec<_> = src_nodes
                .iter()
                .filter(|n| n.node_type == NodeType::Dir)
                .collect();
            let src_files: Vec<_> = src_nodes
                .iter()
                .filter(|n| n.node_type == NodeType::File)
                .collect();
            ...
            let docs_files: Vec<_> = docs_nodes
                .iter()
                .filter(|n| n.node_type == NodeType::File)
                .collect();
...
async fn tree_browser_multi_path_nodes_do_not_cross_contaminate() {
    ...
    let store = shared_test_store().await;
    let result = store
        .list_project_tree_multi(1, &["/src".to_string(), "/docs".to_string()], false)
        .await
        .unwrap();
...
async fn tree_browser_invalid_expand_path_is_detected() {
    ...
    let result = store
        .list_project_tree_multi(1, &["/src".to_string(), "/nonexistent".to_string()], false)
        .await
        .unwrap();
...
async fn list_projects_excludes_canary() {
    ...
    assert!(
        !projects.is_empty(),
        "fixture should contain a real project"
    );
    for p in &projects {
        assert!(
            p.id > 0,
            "non-persistent project leaked into listing: {:?}",
            p
        );
...
async fn get_project_details_excludes_canary() {
    ...
    assert!(
        details.is_none(),
        "canary project must not be fetchable by id"
    );
}
