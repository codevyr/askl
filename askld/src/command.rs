...
impl Command {
    ...
    pub fn get_selector_composite_filter(&self, eph_ids: &[i64]) -> Option<CompositeFilter> {
        let parts: Vec<_> = self.selectors()
            .filter_map(|sel| sel.build_composite_filter(self, eph_ids))
            .collect();
        match parts.len() {
            0 => None,
    ...
    pub async fn notify_from_selection(
        ...
        for selector in self.selectors() {
            ...
            let mut selection = find_symbol_by_instance_id(index, &selector_filters, decl_ids, parent_scope.clone(), children_scope.clone(), &ctx.eph_ids)
                .await
                .map_err(|e| {
                    pest::error::Error::new_from_span(
                        pest::error::ErrorVariant::CustomError {
    ...
    pub async fn compute_selected(
        ...
        let filter_parts: Vec<CompositeFilter> =
            self.filters().filter_map(|f| f.get_composite_filter(eph_ids)).collect();
