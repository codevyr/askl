...
pub async fn query(data: web::Data<AsklData>, req_body: String) -> impl Responder {
    ...
    match execute_query(&data.cfg, &req_body).await {
        Ok(graph) => {
            let json_graph = serde_json::to_string_pretty(&graph).unwrap();
            HttpResponse::Ok().body(json_graph)
        }
        Err(QueryFailure::BadRequest(err)) => {
            let json_err = serde_json::to_string(&err).unwrap();
            HttpResponse::BadRequest().body(json_err)
        }
        Err(QueryFailure::Timeout) => HttpResponse::RequestTimeout().body("Query timed out"),
    }
}
...
pub async fn execute_query(
    ...
    req_body: &str,
) -> Result<Graph, QueryFailure> {
    let ast = match parse(req_body) {
        Ok(ast) => ast,
        Err(err) => {
            println!("Parse error: {}", err);
            return Err(QueryFailure::BadRequest(ErrorResponse {
    ...
    let res = {
        ...
        let execute_future = ast.execute(&mut ctx, cfg);
